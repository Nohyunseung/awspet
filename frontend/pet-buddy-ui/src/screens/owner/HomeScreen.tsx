  ...
  const handleReserveFromJob = async () => {
    ...
    try {
      ...
      if (resp.success) {
        ...
        // 서버 예약 실패 시, 공고 정보를 기반으로 로컬 일정에 추가하고 공고는 닫아 UI를 맞춥니다.
        try {
          if (selectedJob?.post_id) {
            await apiService.post(`/sitter-postings/${selectedJob.post_id}/close`)
            const jobs = await apiService.get<{ posts: any[] }>('/sitter-postings')
            if (jobs.success) setJobFeed((jobs.data as any)?.posts || [])
          }
        } catch {}
        setUpcomingSchedules(prev => {
          const newItem = {
            id: `local-${Date.now()}`,
            dogName: myDogsFromApi.find(d=>d.id===dogId)?.name || '강아지',
            sitterName: selectedJob?.sitter_name || selectedJob?.sitter_email || '시터',
            service: selectedJob?.title || '돌봄 예약',
            date: String((selectedJob?.available_from || selectedJob?.start_date || '')).slice(0,10),
            time: `${String(start).slice(11,16)} - ${String(end).slice(11,16)}`,
            status: 'confirmed',
            location: selectedJob?.location || '-',
            startAt: new Date(String((selectedJob?.available_from || selectedJob?.start_date || '')).slice(0,10) + 'T' + String(start).slice(11,16) + ':00').getTime(),
          }
          return [...prev, newItem].sort((a:any,b:any)=> (a.startAt||0)-(b.startAt||0))
        })
        closeJobDetail()
        Alert.alert('예약(임시) 추가', '서버 저장은 실패했지만 일정에 반영하고 공고를 숨겼습니다.')
      }
    } catch (e: any) {
                ...
                onPress={async () => {
                  ...
                  try {
                    ...
                    if (resp.success) {
                      ...
                      setDogForm({ name: '', breed: '', personality: '', birthDate: '', notes: '', profileImageUrl: '' })
                      // 목록 새로고침 (향후 노출용)
                      const refreshed = await apiService.get<{ dogs: any[] }>(`/dogs/user/${user.id}`)
                      if (refreshed.success) setMyDogsFromApi(refreshed.data?.dogs || [])
        ...
        <SafeAreaView style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.modalPhotoContainer}>
              <Image source={{ uri: selectedDog?.photo }} style={styles.modalPhoto} />
              {isEditing && (
            ...
            <View style={styles.modalSection}>
              ...
              <View style={styles.modalField}>
                <Text style={styles.modalFieldLabel}>나이</Text>
                {isEditing ? (
                  <TextInput
                    style={styles.modalInput}
                    value={editForm.age}
                  ...
                  <Text style={styles.modalFieldValue}>{selectedDog?.age}</Text>
                )}
              </View>

              <View style={styles.modalField}>
                <Text style={styles.modalFieldLabel}>체중</Text>
                {isEditing ? (
                  <TextInput
                    style={styles.modalInput}
                  ...
                  <Text style={styles.modalFieldValue}>{selectedDog?.weight}</Text>
                )}
              </View>
            </View>
            ...
            <View style={styles.modalSection}>
              ...
              <View style={styles.modalField}>
                ...
                {isEditing ? (
                  ...
                  <Text style={styles.modalFieldValue}>{selectedDog?.personality}</Text>
                )}
              </View>

              <View style={styles.modalField}>
                <Text style={styles.modalFieldLabel}>댕BTI</Text>
                {isEditing ? (
                  <View style={styles.dangbtiContainer}>
                    <TextInput
                      ...
                      maxLength={4}
                    />
                    <TouchableOpacity style={styles.dangbtiTestButton}>
                      <Text style={styles.dangbtiTestText}>테스트하기</Text>
                    </TouchableOpacity>
                  </View>
                ) : (
                  <View style={styles.dangbtiDisplayContainer}>
                    <View style={styles.dangbtiBadge}>
                      <Text style={styles.dangbtiText}>{selectedDog?.dangbti}</Text>
                    </View>
                    <Text style={styles.dangbtiDescription}>
                      {getDangBTIDescription(selectedDog?.dangbti)}
                    </Text>
                  </View>
                )}
              </View>
